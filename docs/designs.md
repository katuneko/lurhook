...
* **サウンド再生**: 魚ヒットやライン切断、嵐発生時などに効果音を鳴らし、
  昼夜や嵐の状況に応じたBGMをループ再生する。
...
* インベントリ領域にフォーカス中は`X`で魚を生食して満腹度+20。
* フォーカス中は上下キーでカーソル移動、Enterで選択アイテムを装備/消費する。
