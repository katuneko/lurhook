...
- [x] **インベントリ常時表示パネル** – マップ右側に固定表示し `i` でフォーカス切替。
...
## 7. コンテンツ & リプレイ性
- [x] **探索エリア拡張 (沿岸→沖合→深海)** – 条件達成でアンロック。マップサイズ & 敵リスク増。
- [x] **図鑑システム & 実績** – 捕獲魚の情報を永続保存。Steam実績API互換 JSON を想定。
- [x] **難易度モード** – Easy/Normal/Hard で満腹減衰速度・敵出現率等をスケール。
- [x] **ゲームバランス調整パス** – 上記追加要素を踏まえた総合調整。テストプレイ→数値fix。
